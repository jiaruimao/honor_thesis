...
We projected each subject's preprocessed resting-state fMRI data onto the cortical surface and obtained time series for 20,484 vertices (10,242 per hemisphere). The Hurst exponent (\(H\)) was estimated using a discrete wavelet transform and a fractionally integrated process (FIP) model, with parameters fit by maximum likelihood estimation. This FIP-based estimation differs from our prior fGn-based implementation (Lai et al., 2010): fGn is a special case within the FIP family but assumes stationarity and constrains \(H \le 1\). In practice, this upper bound can induce ceiling effects in infant rs-fMRI; by contrast, the FIP model relaxes stationarity and allows \(H>1\), improving sensitivity when between-subject variability occurs near or above 1. When \(H>1\), the time series is treated as non-stationary with long-memory (fractal) characteristics.

Hurst estimation was implemented using the nonfractal MATLAB toolbox (\url{https://github.com/wonsang/nonfractal}), specifically \texttt{bfn\_mfin\_ml.m} with \texttt{filter='haar'}, \texttt{ub=[1.5,10]}, and \texttt{lb=[-0.5,0]}.
...
\subsection{Myelination calculation}\label{subsec:myelination}
...
\subsection{Statistical framework for spatial, temporal, and pathway analyses}\label{subsec:correlation}
